...
class FileReader {
    ...
    */
    readNodes(root, path) {
        ...
        const cachedText = this._fileCache[resolvedPath]
        
        let textPromise = null

        if (cachedText) {
            textPromise = Promise.resolve(cachedText)
        } else {
            textPromise = this._readFileFromDisk(resolvedPath)
        }

        return textPromise.then((text) => {
            this._fileCache[resolvedPath] = text
