...
class PrecompilingReader extends Reader {
    ...
    constructor(basePath, enableComponentRegistry) {
        super(basePath)

        if (enableComponentRegistry) {
            this._componentRegistry = new ComponentRegistry(basePath)
        }
    ...
    */
    ...
    _resolveComponents(nodes) {
        if (this._componentRegistry) {
            new Nodes(nodes).forEach((root, index) => {
                root.filter((node) => {
                    if (node.type === Node.TYPE_TAG) {
                        const name = node.name
                        if (this._componentRegistry.hasComponent(name)) {
                            const component = this._componentRegistry.getComponent(name)
                            const componentNode = Node.fromString(component.template())

                            if (!node.parent || node.parent.type === 'root') {
                                nodes[index] = componentNode
                            } else {
                                root.replaceChild(componentNode, Node.of(node))
                            }
                        }
                    }
                })
