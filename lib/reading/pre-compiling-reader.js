...
class PrecompilingReader extends Reader {
    ...
    */
            ...
            for (let index = nodes.length - 1; index >= 0; index--) {
                ...
                root.filterByType((node) => {
                    ...
                    if (this._componentRegistry.hasComponent(name)) {
                        ...
                        this._resolveComponentsTemplatesAndSlots(componentNodes, Node.of(node))
                    }
                }, [Node.TYPE_TAG])
            }
        }
    ...
    _resolveComponentsTemplatesAndSlots(componentNodes, htmlPlaceholderNode) {
        const templates = htmlPlaceholderNode.find('template')
        const slots = componentNodes.reduce((current, next) => {
            return current.concat(next.find('slot'))
        }, [])

        if (slots.length !== templates.length) {
            throw createError(`Uneven number of templates and slots. "${htmlPlaceholderNode.toHtml()}" has ${templates.length} templates and component "${this._writer.toHtmlString(componentNodes)}" has ${slots.length} slots.`)
        }
        ...
        const slotsAndTemplatesByName = slots.reduce((current, next) => {
            ...
            if (name && Object.prototype.hasOwnProperty.call(current, name)) {
                throw createError(`There is more than one slot with name "${name}" in "${componentNodes.toHtml}"`)
            }
            if (!name) {
                name = ''
            }
        ...
        Object.entries(slotsAndTemplatesByName).forEach((entry) => {
            entry[1][0].slot.root.replaceChild(entry[1][0].template[0], entry[1][0].slot)
            entry[1][0].template.slice(1).reverse().forEach((template) => {
                entry[1][0].slot.root.appendChildAfter(template, entry[1][0].template[0])
            })
        })
