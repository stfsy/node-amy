...
describe('Parser2', () => {
    ...
    describe('.parseLine', () => {
        ...
        it('should also return add operation', () => {
            const operations = parser.parseLine('@amy import abc/def.html with abc.d.e.f.g as amy and add abc.html with def for x2')
            ...
            expect(operations[2].name()).to.equal('as')
            expect(operations[2].arguments()).to.equal('amy')
            expect(operations[3].name()).to.equal('add')
            expect(operations[3].arguments()).to.equal('abc.html')
            expect(operations[4].name()).to.equal('with')
            expect(operations[4].arguments()).to.equal('def')
            expect(operations[5].name()).to.equal('for')
            expect(operations[5].arguments()).to.equal('x2')
        })
    })
})